...
`Tab` 으로 메타 명령·테이블 이름·SQL 키워드를 완성합니다.
...
나오므로, 실패했을 때 어디까지 갔는지도 알 수 있습니다.

두 경우에만 내지 않습니다.

- **아무 구간도 실행되지 않았을 때** — 패키지가 없거나 인자가 틀려서 시작도 못 한
  경우입니다. `합계 0.000초` 만 있는 표는 알려주는 것이 없습니다.
- **대화형 셸** — 세션 전체를 재봐야 의미가 없습니다. 문장별 시간은 셸 안에서
  `\timing` 이 맡습니다.
...
**그냥 PostgreSQL 에 붙어도 동작합니다.** 분산키 함수는 Greenplum 에만 있어서,
접속할 때 있는지 확인해두고 없으면 그 줄을 빼고 만듭니다. `\d` 의 분산키 표시도
같습니다. 없는 함수는 `CASE` 안에 넣어두기만 해도 문장 전체가 해석 단계에서
실패하므로, SQL 을 만들기 전에 알아야 합니다.
...
| --- | --- |
| `\d` | 메타 명령 (`\d`, `\dt`) |
| `order` | 테이블 이름 먼저, 그다음 SQL 키워드 (`orders`, `order_items`, `ORDER BY`) |

**테이블 이름을 키워드보다 앞에 둡니다.** 키워드는 외우고 있지만 이름은 그렇지
않습니다. Greenplum 은 스키마를 붙인 이름(`staging.orders`)과 안 붙인 이름
(`orders`)을 모두 완성합니다.

테이블 목록은 **처음 `Tab` 을 누를 때 한 번만** 받아둡니다. 카탈로그 조회가 느릴 수
있어서입니다. 새로 만든 테이블이 안 보이면 `\dt` 로 목록을 다시 받으세요. 캐시도
함께 갱신됩니다.

자동완성이 실패해도 조용히 넘어갑니다. 조회 권한이 없다고 셸이 시끄러워질 이유는
없습니다.

`readline` 이 없는 환경(윈도우, 최소 도커 이미지)에서는 자동완성과 히스토리 없이
동작합니다.

### 긴 결과 보기

결과가 터미널 높이를 넘으면 페이저로 넘깁니다. `$PAGER` 를 쓰고, 없으면
`less -FRSX` 입니다.
...
| `q` | 나가기 |
| `Space` / `b` | 한 화면 아래 / 위 |
| `←` `→` | 좌우 스크롤 — 컬럼이 많아 오른쪽이 잘릴 때 |
| `g` / `G` | 맨 위 / 맨 아래 |
| `/단어` | 검색 (`n` 으로 다음) |

`$PAGER` 를 다른 것으로 지정해 뒀다면 그 프로그램의 키를 따릅니다.

기본값 `less -FRSX` 의 각 옵션은 이렇습니다.

| 옵션 | 뜻 |
| --- | --- |
| `-F` | 한 화면에 들어가면 페이저 없이 그냥 출력 |
| `-R` | 색 그대로 |
| `-S` | 긴 줄을 접지 않고 옆으로 (컬럼이 많을 때). 좌우 스크롤이 필요한 이유입니다 |
| `-X` | 끝난 뒤 화면을 지우지 않음 — 결과가 스크롤백에 남습니다 |

```
dw=> \pager off     # 항상 그냥 출력
dw=> \pager on      # 항상 페이저
dw=> \pager         # 지금 설정 보기
```

기본은 `auto` 입니다. **화면에 들어가는 결과까지 페이저를 거치면 오히려 성가시므로**
터미널 높이를 넘을 때만 씁니다. 터미널이 아니거나(파이프, 크론) `\o` 로 파일에
쓰는 중이면 쓰지 않습니다.

`\dt` 는 테이블이 많으면 거의 항상 페이저를 탑니다. 목록만 훑을 거라면 패턴으로
줄이는 편이 낫습니다.
